...
if not df.empty:
    ...
    if missing_cols:
        ...
        if search_phrase:
            ...
            st.info("Enter a search phrase above to start searching, or use the exploration buttons below.")
            
            # --- Exploration Buttons ---
            # Create two columns to place the buttons side-by-side
            col1, col2 = st.columns(2) 

            # Button 1: Show Top 1000 Tweets (Original functionality)
            with col1:
            ...
            # Button 2: Show Random Tweets (NEW FEATURE)
            with col2:
                if st.button("Display 100 Random Tweets 🎲"):
                    # Determine sample size
                    sample_size = min(100, len(df))
                    
                    # Use pandas .sample() to get a new random subset of 100 rows on every click
                    random_sample_df = df.sample(n=sample_size)[COLUMNS_TO_DISPLAY].copy()
